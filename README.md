# 介绍
用于分析`iOS`ipa包的脚本工具，使用此工具可以`自动扫描发现`可修复的包体积问题，同时可以生成包体积数据用于查看。希望可以帮助到更多的开发同学更加效率的优化包体积问题。

## 背景
`APPAnalyze`工具最早诞生主要是为了解决以下包体积管理的问题：

对于很多`APP`来讲，包体积是一个非常重要的性能指标，包体积过大会影响用户下载`APP`的意愿。但是在早期我们缺少一些手段帮助我们更高效的去进行包体积管理。
#### 自动发现问题
